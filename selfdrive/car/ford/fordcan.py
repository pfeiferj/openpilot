...
def create_acc_msg(packer, CAN: CanBus, long_active: bool, gas: float, accel: float, stopping: bool, v_ego_kph: float):
  ...
  values = {
    ...
    "AccPrpl_A_Pred": lh.target_accel if long_active else -5.0,                           # Acceleration request: [-5|5.23] m/s^2
    "AccResumEnbl_B_Rq": 1 if long_active else 0,
    "AccVeh_V_Trg": lh.target_speed * CV.MS_TO_KPH if long_active else v_ego_kph,                        # Target speed: [0|255] km/h
    # TODO: we may be able to improve braking response by utilizing pre-charging better
    "AccBrkPrchg_B_Rq": 1 if lh.precharge and long_active else 0,            # Pre-charge brake request: 0=No, 1=Yes
